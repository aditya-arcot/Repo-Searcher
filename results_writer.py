...
class ResultsWriter:
    ...
    def write_config(self, config:ConfigurationManager, search_repos_branches:dict) -> None:
        ...
        included_repos_str = self.__config_str('Included repos / branches', config.get_dict(ConfigEnum.INCLUDED_REPOS.name))
        excluded_repos_str = self.__config_str('Excluded repos / branches', config.get_dict(ConfigEnum.EXCLUDED_REPOS.name))

        search_template_mode = SearchTemplateModeEnum(config.get_int(ConfigEnum.SEARCH_TEMPLATE_MODE.name)).name
        search_template_mode_str = self.__config_str('Search template mode', search_template_mode)

        search_repos_branches_str = self.__config_str('Search repos / branches', search_repos_branches)

        search_words_str = self.__config_str('Search words', config.get_list(ConfigEnum.SEARCH_WORDS.name))

        excel_search_mode = SearchExcelModelEnum(config.get_int(ConfigEnum.EXCEL_SEARCH_MODE.name)).name
        excel_search_mode_str = self.__config_str('Excel search mode', excel_search_mode)

        excluded_files_str = self.__config_str('Excluded files', config.get_list(ConfigEnum.EXCLUDED_FILES.name))
        excluded_folders_str = self.__config_str('Excluded folders', config.get_list(ConfigEnum.EXCLUDED_FOLDERS.name))
    ...
    def __config_str(self, heading:str, config_item=None) -> str:
        ...
        if isinstance(config_item, list):
            ...
            return Constants.TAB + heading + Constants.NEWLINE + (Constants.TAB * 2) + config_item
    ...
    def write_branch_results(self, repo:str, branch:str, results:BranchSearchResults) -> None:
        ''' writes all search results sections '''
        
        n, lines = self.__format_matches_section(results.matches)
        if lines:
            self.__write_lines(self.__matches_file, 
                               [repo, f'{Constants.TAB}{branch} ({n})'] + lines)
    ...
    def __format_matches_section(self, branch_matches:dict) -> Tuple[int, Optional[List[str]]]:
        count = 0
