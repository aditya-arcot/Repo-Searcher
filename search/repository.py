...
class ADORepository:
    ...
    def __init__(self, logger:LoggingManager, name:str,
                 branches:set[str], url:str, path:str) -> None:
        self.logger = logger
    ...
    def update_branch(self, branch, last_update_dict:dict):
        ...
        if os.path.exists(os.path.join(self.path, branch)):
            ...
            if self.name not in last_update_dict \
                or branch not in last_update_dict[self.name] \
                or time.time() - last_update_dict[self.name][branch] > self.day_in_seconds:
                self.logger.info('update required')
                return self.__update_helper(branch, last_update_dict, GitCommandEnum.PULL.name)
    ...
    def __attempt_update(self, branch, mode) -> bool:
        ...
        try:
            if mode == GitCommandEnum.PULL.name:
                ...
                Repo.clone_from(self.url, os.path.join(self.path, branch), branch=branch)
